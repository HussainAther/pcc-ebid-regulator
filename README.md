...
The large direct-control result from Experiment 025 was decomposed with a stronger nonlinear state baseline and an exact endpoint-entropy sanity control. Once current-state entropy is already represented exactly, historical entropy (`H_initial`, `H_mean`) adds about **4.3% pooled leave-one-class-out MAE reduction**, and the remaining canonical rate terms add another **2.4%**. PCC retains a larger ~**7.3%** history contribution, but neutral diffusion is harmed by history. Thus the large 025 effect was partly representational: genuine temporal entropy information survives, but is smaller and mechanism-dependent. See `experiments/026_entropy_history_decomposition/RESULTS.md`.
